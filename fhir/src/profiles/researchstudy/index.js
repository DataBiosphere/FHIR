...
const { getQueryStandardParameters, standardParameters, getSearchParameters } = require('../../utils/searching');
...
const search = async ({ base_version: baseVersion }, { req }) => {
  ...
  if (_source) {
    ...
    const promises = [];
    promises.push(tcga.getAllResearchStudy({ offset: currentOffsets.tcga, search: searchParams, ...params }));
    promises.push(anvil.getAllResearchStudy({ offset: currentOffsets.anvil, search: searchParams, ...params }));
